#!/bin/bash
cat << 'INNEREOF' > temp_settings.txt
...
fun PrinterSettingsDialog(viewModel: MainViewModel, onDismiss: () -> Unit) {
    ...
    LaunchedEffect(Unit) {
        try {
            pairedDevices = com.example.logic.BluetoothPrinter.getPairedDevices()
        } catch (e: Exception) {}
    }
            ...
            Column(modifier = Modifier.fillMaxWidth()) {
                ...
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(
                        ...
                        colors = ButtonDefaults.buttonColors(containerColor = if (paperSize == "58mm") MaterialTheme.colorScheme.primary else Color.Gray)
                    ) { Text("58mm") }
                    Button(
                        onClick = { paperSize = "80mm" },
                        colors = ButtonDefaults.buttonColors(containerColor = if (paperSize == "80mm") MaterialTheme.colorScheme.primary else Color.Gray)
                    ) { Text("80mm") }
                }
                ...
                if (pairedDevices.isEmpty()) {
                    Text("တွဲထားသော (Paired) ပရင်တာ မတွေ့ပါ", color = Color.Red)
                } else {
                        ...
                        items(pairedDevices.size) { i ->
                            ...
                            Row(
                                modifier = Modifier
                                    ...
                                    .padding(8.dp)
                                    .background(if (macAddress == device.address) MaterialTheme.colorScheme.primaryContainer else Color.Transparent),
                ...
                Button(
                    onClick = {
                        ...
                        kotlinx.coroutines.GlobalScope.launch(kotlinx.coroutines.Dispatchers.Main) {
                            val success = com.example.logic.BluetoothPrinter.connect(macAddress)
                            isConnecting = false
...
sed -i '/fun PrinterSettingsDialog/,/^}/d' app/src/main/java/com/example/ui/SettingsScreen.kt
cat temp_settings.txt >> app/src/main/java/com/example/ui/SettingsScreen.kt
